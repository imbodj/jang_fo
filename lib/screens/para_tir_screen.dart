...
class ParaTirSoundManager {
  ...
  static Future<void> loadMutePref() async {
    ...
    muted = prefs.getBool('paratir_muted') ?? false;
  }

  static Future<void> toggleMute() async {
    muted = !muted;
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool('paratir_muted', muted);
  }
...
class ParaTirMainMenuScreen extends StatefulWidget {
  const ParaTirMainMenuScreen({super.key});

  @override
  State<ParaTirMainMenuScreen> createState() => _ParaTirMainMenuScreenState();
}

class _ParaTirMainMenuScreenState extends State<ParaTirMainMenuScreen> {
  int highScore = 0;
  int bestLevel = 1;
  List<int> topScores = [];
  ...
  Future<void> _loadPrefs() async {
    await ParaTirSoundManager.loadMutePref();
    final prefs = await SharedPreferences.getInstance();
    setState(() {
                          ...
                          ElevatedButton(
                            onPressed: () {
                              Navigator.push(
                                ...
                                MaterialPageRoute(
                                    builder: (context) =>
                                        ParaTirGameScreen(skin: selectedSkin)),
                              ).then((_) => _loadPrefs());
                            },
              ...
              Positioned(
                ...
                child: Row(
                  children: [
                    ...
                    IconButton(
                      icon: Icon(
                          ParaTirSoundManager.muted
                              ? Icons.volume_off
                              : Icons.volume_up,
                          color: Colors.amberAccent,
                          size: 28),
                      onPressed: () async {
                        await ParaTirSoundManager.toggleMute();
                        setState(() {});
                      },
                    ),
                    IconButton(
  ...
  Widget _skinButton(CannonSkin skin, Color color) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isSelected ? color.withValues(alpha: 0.3) : Colors.black26,
...
class ParaTirGameScreen extends StatefulWidget {
  final CannonSkin skin;
  const ParaTirGameScreen({super.key, required this.skin});

  @override
  State<ParaTirGameScreen> createState() => _ParaTirGameScreenState();
}

class _ParaTirGameScreenState extends State<ParaTirGameScreen>
  ...
  void createExplosion(double x, double y, Color color) {
    ParaTirSoundManager.playExplosion();
    ...
    setState(() {
      ...
      powerUps.removeWhere((pu) {
        if ((pu.x - cannonX).abs() < 0.1 && pu.y >= 0.8) {
          tripleShotTicks = 300;
          ParaTirSoundManager.playPowerUp();
